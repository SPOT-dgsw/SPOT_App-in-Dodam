  ...
  return (
    <div className="cu-page max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={goBack} className="cu-btn cu-btn-muted !px-2">
          &larr;
        </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">
            결재 양식 <span style={{ color: 'var(--cu-danger)' }}>*</span>
          </label>
          <select
            value={template}
        ...
        <div>
          <label className="block text-sm font-medium mb-1">
            제목 <span style={{ color: 'var(--cu-danger)' }}>*</span>
          </label>
          <input
            type="text"
            value={title}
        ...
        <div>
          ...
          {approver ? (
            <div className="flex items-center gap-2">
              <span
                ...
                style={{ background: 'var(--cu-panel-soft)', color: 'var(--cu-text)' }}
              >
                {approver.name}
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {showDropdown && memberResults.length > 0 && (
                <ul
                  ...
                  style={{ background: 'var(--cu-panel)', borderColor: 'var(--cu-line)' }}
                >
                  {memberResults.map((m) => (
                    <li key={m.id}>
                      <button
              ...
              {showDropdown && memberResults.length === 0 && memberQuery.trim() && (
                <div
                  className="absolute z-50 w-full mt-1 rounded-lg shadow-lg p-3 text-sm border"
                  style={{ background: 'var(--cu-panel)', borderColor: 'var(--cu-line)', color: 'var(--cu-muted)' }}
                >
                  검색 결과가 없습니다.
                </div>
              )}
          ...
          <p className="mt-1 text-xs" style={{ color: 'var(--cu-muted)' }}>
          ...
          <div data-color-mode="light">
            <div className="flex items-center justify-between mb-1">
              <label className="block text-sm font-medium">
                내용 <span style={{ color: 'var(--cu-danger)' }}>*</span>
              </label>
              <span className="text-xs" style={{ color: content.length > MAX_CONTENT_LENGTH ? 'var(--cu-danger)' : 'var(--cu-muted)' }}>
        ...
        {template && (
          <div>
            <label className="block text-sm font-medium mb-1">
              첨부 파일{' '}
              <span className="text-xs font-normal" style={{ color: 'var(--cu-muted)' }}>
              ...
              <ul className="mt-2 space-y-1">
                {files.map((f, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm">
                    <span className="flex-1 truncate" style={{ color: 'var(--cu-muted)' }}>{f.name}</span>
                    <span className="text-xs shrink-0" style={{ color: 'var(--cu-muted)' }}>
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-4">
          ...
          <button type="button" onClick={goBack} className="cu-btn cu-btn-muted flex-1">
            취소
          </button>
        </div>
      </form>
