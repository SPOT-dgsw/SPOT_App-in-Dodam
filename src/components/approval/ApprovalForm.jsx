...
export default function ApprovalForm({ onSubmit, onCancel, loading }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {/* 양식 선택 */}
      <div>
        <label className="block text-sm font-medium mb-1">결재 양식 <span style={{ color: 'var(--cu-danger)' }}>*</span></label>
        <select
          value={template}
      ...
      {/* 제목 */}
      <div>
        <label className="block text-sm font-medium mb-1">제목 <span style={{ color: 'var(--cu-danger)' }}>*</span></label>
        <input
          type="text"
          value={title}
      ...
      {template && (
        <div>
          <label className="block text-sm font-medium mb-1">
            첨부 파일 <span className="text-xs font-normal" style={{ color: 'var(--cu-muted)' }}>(최대 3개, 각 5MB 이하)</span>
          </label>
          <input
            ref={fileInputRef}
            type="file"
      ...
      <div className="flex flex-col gap-2 pt-2">
        ...
        <button type="button" onClick={onCancel} className="cu-btn cu-btn-muted">
          취소
        </button>
      </div>
    </form>
