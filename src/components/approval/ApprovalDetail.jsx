...
export default function ApprovalDetail({ approval, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3 p-5 border-b" style={{ borderColor: 'var(--cu-line)' }}>
        ...
        <div className="p-5 space-y-5">
          <div className="cu-subcard p-3 space-y-1 text-sm">
            ...
            {approval.resolved_at && (
              <div className="flex gap-2">
                <span className="w-16 shrink-0" style={{ color: 'var(--cu-muted)' }}>처리일시</span>
                <span>{formatDateTime(approval.resolved_at)}</span>
              </div>
            )}
            {approval.approver && (
              <div className="flex gap-2">
                <span className="w-16 shrink-0" style={{ color: 'var(--cu-muted)' }}>결재자</span>
              ...
              <div className="space-y-2">
                {contentEntries.map(([key, value]) => (
                  <div key={key} className="text-sm">
                    <span className="font-medium" style={{ color: 'var(--cu-muted)' }}>
              ...
              <ul className="space-y-1">
                {approval.attachments.map((att) => (
                  <li key={att.id} className="flex items-center justify-between gap-2 text-sm">
                    <span className="truncate" style={{ color: 'var(--cu-muted)' }}>{att.file_name}</span>
